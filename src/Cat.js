...
const Cat = () => {
  ...
  useEffect(() => {
    const bgChangeInterval = setInterval(() => {
      setColorIdx((prevIndex) => ++prevIndex % COLORS.length);
    }, 5000);
    return () => clearInterval(bgChangeInterval);
  }, []);

  //cat Status saves to local storage
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!VALID_STATUS_CODES.includes(Number(statusChange))) {
      alert(
        `Code ${status} might exists but it is not a proper cat status code`
      );
      setStatusChange("404");
    }
  }, [statusChange]);
